...
import { useProducts, useSuppliers, useUpdateProduct } from "@/lib/db";
import type { StatusOverride } from "@/lib/types";

export const Route = createFileRoute("/_authenticated/magazzino/$productId")({
  ...
  return (
    <div className="flex min-h-[100dvh] flex-col bg-background">
      <TopBar
        ...
        showMenu={false}
        right={
          <button onClick={() => navigate({ to: "/magazzino" })} className="text-sm font-medium text-primary">
            Annulla
          </button>
        }
      />
      <main className="mx-auto w-full max-w-screen-sm flex-1 px-4 py-5">
        <button onClick={() => navigate({ to: "/magazzino" })} className="mb-4 inline-flex items-center gap-1 text-sm text-muted-foreground">
        ...
        <div className="mb-5 rounded-2xl border border-border bg-card p-4 shadow-sm">
          ...
          <div className="mt-3">
            <TrafficBadge
              product={{
                ...product,
                current_stock: stock === "" ? null : parseInt(stock, 10) || 0,
                min_threshold: parseInt(min, 10) || 0,
                manual_status_override: override,
              }}
            />
          </div>
        ...
        <Section label="Giacenza attuale"><NumField value={stock} onChange={setStock} unit={product.unit} /></Section>
        <Section label="Soglie">
          <div className="grid grid-cols-2 gap-3">
            <div><Label>Minima</Label><NumField value={min} onChange={setMin} unit={product.unit} /></div>
            <div><Label>Massima</Label><NumField value={max} onChange={setMax} unit={product.unit} /></div>
          </div>
        </Section>
        <Section label="Stato manuale">
          <div className="grid grid-cols-4 gap-2">
            {(["AUTO", "GREEN", "YELLOW", "RED"] as const).map((opt) => {
              ...
              const styles =
                opt === "AUTO" ? "border-border"
                : opt === "GREEN" ? "border-[var(--status-green)] text-[var(--status-green)]"
                : opt === "YELLOW" ? "border-[var(--status-yellow)] text-[var(--status-yellow)]"
                : "border-[var(--status-red)] text-[var(--status-red)]";
              return (
                <button
                  key={opt}
                  onClick={() => setOverride(opt === "AUTO" ? null : opt)}
                  className={`h-11 rounded-xl border-2 text-xs font-semibold transition-colors ${styles} ${selected ? "bg-muted" : "bg-card"}`}
                >
        ...
        <button
          ...
          className="mt-6 h-12 w-full rounded-2xl bg-primary text-sm font-semibold text-primary-foreground shadow-sm transition-colors hover:bg-primary/90 disabled:opacity-60"
        >
          Salva modifiche
        </button>
      </main>
